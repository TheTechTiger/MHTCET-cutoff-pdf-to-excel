            ...
            for page_idx in range(pages_to_process_actually):
                ...
                while line_idx < len(lines):
                    ...
                    if course_match and current_college_code:
                        ...
                        # current_level is now determined by the new dynamic logic before college/course matching.
                        # The old explicit skip for "State Level" after status is no longer needed here.
                        # if line_idx < len(lines) and lines[line_idx].strip() == "State Level":
                        #     line_idx += 1
                        ...
                        logging.info(f"Page {current_page_num}: Potential Categories: {actual_category_codes} for course {current_course_name}")
                        ...
                        if line_idx < len(lines):
                            ...
                            if initial_data_match:
                                ...
                                logging.debug(f"Page {current_page_num}: No initial data line starting with 'I' found for course {current_course_name} at line: {lines[line_idx].strip() if line_idx < len(lines) else 'EOF'}")

                        actual_rank_percentiles = rank_percentile_re.findall(data_values_text.strip())
                        logging.info(f"Page {current_page_num}: Potential Ranks/Percentiles: {actual_rank_percentiles} for course {current_course_name} from text: '{data_values_text}'")
                        ...
                        for i, category_code in enumerate(actual_category_codes):
                            if i < len(actual_rank_percentiles):
                                ...
                                row = [
                                    str(serial_number),
                                    str(current_page_num), # Use current_page_num from loop
                                ...
                                logging.warning(f"Page {current_page_num}: Mismatched data for {current_course_name}: Cat: {category_code} at index {i}, but only {len(actual_rank_percentiles)} R/P pairs.")
...
if __name__ == "__main__":
    ...
    if not os.path.exists(pdf_file_path_main):
        ...
        # Call with number of pages to process
